...
const paramSum = (v1, v2) => sum(v1(), v2());

const positionMode = (index, program) => {
    return program[program[index]];
};

const immediateMode = (index, program) => {
    return program[index];
};

type ParameterMode = (index, program) => number;


function determineOperation(index: number, program: number[]) {
    const operation = program[index];

    if (operation === 3) {
        return {
            execute: () => {
                const resultIndex = program[index + 1];
        ...
        return {
            execute: () => {
                ...
                program[indexResult] = sum(program[indexValue1], program[indexValue2]);
            },
            nextIndex: index + 4
        };
    } else if (operation === 2) {
        return {
            execute: () => {
                ...
                let indexValue2 = program[index + 2];
                let indexResult = program[index + 3];
                program[indexResult] = multiply(program[indexValue1], program[indexValue2]);
            },
            nextIndex: index + 4
        };
    } else if (operation === 4) {
        return {
            execute: () => {
                const indexResult = program[index + 1];
                console.log(program[indexResult]);
            },
            nextIndex: index + 2
        };
    } else {
...
const handleOperation = (program: number[], index: number = 0) => {
    ...
    const operation = determineOperation(index, program);
    operation.execute();

    return handleOperation(program, operation.nextIndex);
};

const result = handleOperation(program);
